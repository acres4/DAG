...
with DAG(
    ...
    def run_dbt(conn_env):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            # Get the path to dbt executable in virtualenv
            venv_bin_path = os.path.dirname(os.sys.executable)
            dbt_executable = os.path.join(venv_bin_path, 'dbt')

            # Ensure it's executable
            os.chmod(dbt_executable, 0o755)

            # Run dbt
            result = subprocess.run(
                [dbt_executable, "run", "--profiles-dir", dbt_project_dir, "--project-dir", dbt_project_dir],
                env=env,
                capture_output=True,
                text=True,
            )
